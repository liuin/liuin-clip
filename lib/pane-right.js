...
module.exports = MySelectListView = (function(superClass) {
  ...
  MySelectListView.prototype.reloadFIle = function(){
    ...
    fs.readdir('E:/xampp/htdocs/liuin-clip/snippets/', function(err, files) {
      if (err) {
        console.log(err);
        return;
      }
  ...
  MySelectListView.prototype.viewForItem = function(item) {
    ...
    return "<li data-path='"+'E:/xampp/htdocs/liuin-clip/snippets/'+item+'.cson'+"'><span class=\"lab\">" + item + "</span> " + buttonMod + " " + buttonDel + "</li>";
  };
